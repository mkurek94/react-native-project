      ...
      <View className="flex-row gap-3 items-start">
        <View className="justify-center items-center flex-row flex-1">
          ...
          <View className="justify-center gap-y-1 flex-1 ml-3">
            ...
            <Text className="text-xs text-gray-100 font-pregular" numberOfLines={1}>{item.creator?.username}</Text>
          </View>
        </View>
        <View className="pt-2">
        ...
        {play ? <Text className="text-white">Playing</Text> : (
            <TouchableOpacity className="w-full h-60 rounded-xl mt-3 relative justify-center items-center" activeOpacity={0.7} onPress={() => setPlay(true)}>
                <Image source={{uri: item.thumbnail}} className="w-full h-full rounded-xl mt-3" resizeMode="cover"/>
